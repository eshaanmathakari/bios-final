...
ggplot(cor_df, aes(x = Image1, y = Image2, fill = Similarity)) +
  ...
  labs(title = "Heatmap of Image Similarities", x = "Image", y = "Image")
```
...
ui <- fluidPage(
  titlePanel("Interactive Dashboard: Curated X-Ray Dataset"),
  
  sidebarLayout(
    sidebarPanel(
      selectInput("category", "Select a Category:", 
                  choices = c("COVID-19", "Normal", "Bacterial", "Viral")),  # Dropdown menu
      sliderInput("num_images", "Number of Images to Display:", 
                  min = 1, max = 10, value = 5),  # Slider for selecting number of images
      actionButton("update", "Update Images")  # Button to update images
    ),
    
    mainPanel(
      h3("Sample Images"),
      uiOutput("image_display"),  # Dynamic display of images
      
      h3("Category Distribution"),
      plotOutput("bar_chart")  # Bar chart showing class distribution
    )
  )
)
...
server <- function(input, output, session) {
  pneumonia_datapath <- "/Users/tanyageorge/Documents/GitHub/bios-final/Curated X-Ray Dataset"  # Update this with your dataset path
  
  # Reactive data for image paths
  image_paths <- reactive({
    files <- list.files(file.path(pneumonia_datapath, input$category), full.names = TRUE)
    validate(
      need(length(files) > 0, "No images found in the selected category!")
    )
    files[1:input$num_images]  # Limit to the number selected by the slider
  })
  
  # Display Sample Images
  output$image_display <- renderUI({
    paths <- image_paths()
    tagList(lapply(paths, function(img_path) {
      tags$img(src = img_path, style = "height: 300px; margin: 10px;")  # Adjust size and spacing
    }))
  })
  ...
  output$bar_chart <- renderPlot({
    pneumonia_labels <- c("COVID-19", "Normal", "Bacterial", "Viral")
    image_counts <- sapply(pneumonia_labels, function(label) {
      length(list.files(file.path(pneumonia_datapath, label), full.names = TRUE))
    })
    image_counts_df <- data.frame(Category = pneumonia_labels, Count = image_counts)
    
    ggplot(image_counts_df, aes(x = Category, y = Count, fill = Category)) +
      geom_bar(stat = "identity") +
      theme_minimal() +
      labs(title = "Category Distribution", x = "Category", y = "Number of Images") +
